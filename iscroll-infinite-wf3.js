...
// INSERT POINT: _end
	...
	updateMaxScrollY:function() //wf add
	{
		var limit;
		if ( true ) {
			limit = -Math.max(this.delegate.onScrollHeight(0),this.delegate.onScrollHeight(1))+this.wrapperHeight;
		}
...
// INSERT POINT: _wheel
	...
	midIndexbinSearch:function(midy,lr){
		var low = 0 , high = this.delegate.getDataCount(lr)-1 ;
		while(low < high )
		{
			var mid = Math.ceil((low+high)/2) ;
			if( this.delegate.onCellY(mid,lr) > midy )
				high = mid-1 ;
			else if( this.delegate.onCellY(mid,lr) < midy )
				low = mid + 1;
		}
		return high ;
	},
	...
	reorderInfinite: function () {
		...
		{
			if( this.lastDataCount[icol] > del.getDataCount(icol) )
			{
				this.cleanCells(icol) ;
			}
			var len1 = del.getDataCount(icol);
			if(len1>0)
			{
				var middleIndex = this.midIndexbinSearch(wmsY,icol) ;
				var vis0 = Math.max(middleIndex-7,0) ;
				var vis1 = Math.min(middleIndex+7,del.getDataCount(icol)-1) ;
				if( vis1 == del.getDataCount(icol)-1 )
					if(this.delegate.onCellY(vis1,icol)+this.delegate.onCellHeight(vis1,icol) < wtsY-300 )
					{ vis0 = 0 ;	vis1 = -1 ;}
				for(var iv = vis0 ; iv<=vis1 ; iv++ )
				{
					var cind = this.delegate.onCellIndex15(iv,icol) ;
					if( this.lrCells[icol][cind].dindex!=iv )
					{
						this.lrCells[icol][cind].dindex=iv ;
						this.lrCells[icol][cind].style.height = this.delegate.onCellHeight(iv,icol) + 'px';
						this.delegate.onData(this.lrCells[icol][cind],iv,icol) ;
					}	
					var cy = this.delegate.onCellY(iv,icol) ;
					if ( this.options.infiniteUseTransform ) {
						this.lrCells[icol][cind].style[utils.style.transform] = 'translate(0, ' + cy + 'px)' + this.translateZ;
					} else {
						this.lrCells[icol][cind].style.top = cy + 'px';
					}
				}
			}
			this.lastDataCount[icol] = del.getDataCount(icol) ;
		}
		var sbtm = Math.max(Math.max(this.delegate.onScrollHeight(0),this.delegate.onScrollHeight(1)),this.wrapper.offsetHeight) ;
	...
	lastDataCount:[0,0],
	cleanCells:function(lr){
		for(var i = 0 ; i<15 ; i++ )
		{
			this.lrCells[lr][i].innerHTML = '' ;
			this.lrCells[lr][i].dindex = -1 ;
		}	
	},
